# -*- coding: utf-8 -*-
"""
Created on Tue Mar 16 20:45 2019

@author: Anant Kumar
Workdirectory: /Users/Desktop/Hult/Machine_Learning/Assignment2
GitHub: https://github.com/KumarAnant/GameOfThrone_Challenge/blob/master/GoT.py
Purpose: Predicting isAlive on Game of Thrones dataset

AGENDA:
    01)  Initial Data Set-up
    02)  Data Exploration
    03)  Imputing Missing Values
    04)  Flagging Outliers 
    05)  Correlation Analysis
    06)  Factorization & Dummy Variables
    07)  Feature Engineering
    08)  OLS (Ordinary Least Square) Method Full Model    
    09)  KNN Model
    11)  Random Forest
    12)  Support Vector Classifier
    13)  Gradient Boosting Machine
    14)  Hyper parameter grid search
"""
...
plt.figure(figsize=(30, 20))
plt.title('GoT isAlive Pred Characteristic')
plt.plot(fpr, tpr, 'b', label = 'KNN AUC = %0.3f' % roc_auc)
plt.legend(loc = 'lower right')
plt.plot([0, 1], [0, 1],'r--')
plt.xlim([0, 1])
plt.ylim([0, 1])
...
plt.figure(figsize=(30, 20))
plt.title('GoT isAlive Pred Characteristic')
plt.plot(fpr, tpr, 'b', label = 'RFC AUC = %0.3f' % roc_auc)
plt.legend(loc = 'lower right')
plt.plot([0, 1], [0, 1],'r--')
plt.xlim([0, 1])
plt.ylim([0, 1])
...
plt.figure(figsize=(30, 20))
plt.title('GoT isAlive Pred Characteristic')
plt.plot(fpr, tpr, 'b', label = 'SVC AUC = %0.3f' % roc_auc)
plt.legend(loc = 'lower right')
plt.plot([0, 1], [0, 1],'r--')
plt.xlim([0, 1])
plt.ylim([0, 1])
